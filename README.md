...
- **細胞株名をクリックすると細胞株データベース ([Cellosaurus](https://www.cellosaurus.org/)) が開きます。**
  ...
  判らなければ名前検索へリンクします。
- **遺伝子名をクリックすると [Human Protein Atlas](https://www.proteinatlas.org/) の
  該当遺伝子ページが開きます。** Ensembl ID が判っていれば
  `/<Ensembl ID>-<シンボル>` へ直接、判らなければ HPA の検索へリンクします。
...
5. ヒートマップ上でセルにホバーすると詳細、**細胞株名をクリックで Cellosaurus**、
   **上端の遺伝子名をクリックで Human Protein Atlas**。
   テーブルビューでは遺伝子の列見出しの `↗` が HPA へのリンクです
   （見出し自体のクリックは従来どおり並び替えなので、リンクは別のターゲットに
   してあります）。
...
- **左の絞り込みパネルは折りたためます。** ツールバー左端の `絞り込み` ボタンで
  開閉し、閉じるとヒートマップが幅いっぱいに広がります。状態はブラウザに記憶され、
  次回も同じ状態で開きます。
- **ツールバーのラベルはリストの中の見出しです。** 横にラベルを置く代わりに、
  ドロップダウンを開いたときの先頭に太字の見出し（`並び順` など）が出ます。
  見出しは選択できません。ラベルの分の幅が空いたぶん、コントロール同士の
  間隔を広く取っています。
- **上のタイトルバーはスクロールで隠れます**（固定表示しません）。一度読めば足りる
  情報なので、縦の領域はシートに使います。
