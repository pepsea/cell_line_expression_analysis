...
由来臓器で並べているときは、**臓器ごとにブロックが空白で区切られます**。
区切り線ではなく余白で分けているのは、行の高さが 16px まで詰まると線は見落とされ、
シート全体がひと続きのリストに見えてしまうためです（実際にそう読まれ、臓器ラベルが
隣のブロックのものと取り違えられました）。臓器名は自分のブロックの中にだけ描かれ、
ブロックからはみ出すことはありません。
