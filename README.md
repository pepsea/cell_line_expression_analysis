...
| **全遺伝子の正規化平均** | 遺伝子ごとに 0–1 へ正規化してから平均 | 全体として高い細胞株。発現量の大きい遺伝子に偏りません |
| **全遺伝子の平均（絶対値）** | 生の発現量の算術平均 | 絶対量での比較。ただし GAPDH のような高発現遺伝子に強く左右されます |

正規化は各遺伝子について `log10(1+値) ÷ log10(1+その遺伝子の最大値)`（最大値は
**現在絞り込まれている細胞株の中での**最大値）です。これにより、ハウスキーピング
遺伝子と低発現マーカーが同じ重みで効きます。
選択範囲全体で 0 の遺伝子は情報を持たないため集計から除外します
